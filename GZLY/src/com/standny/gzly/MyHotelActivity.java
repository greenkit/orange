...
public class MyHotelActivity extends MasterActivity implements OnScrollListener {
	static final String TAG = "MyHotel";
	...
	// 这里接收支付结果，支付宝手机端同步通知
	public Handler mHandler = new AlipayHandler<MyHotelActivity>(
			MyHotelActivity.this) {
				...
				case AlixId.RQF_PAY: {
					...
					try {
						...
						if (retVal == ResultChecker.RESULT_CHECK_SIGN_FAILED) {
							...
							{
								BaseHelper.showDialog(MyHotelActivity.this,
						...
						e.printStackTrace();
						BaseHelper.showDialog(MyHotelActivity.this, "提示", ret,
								R.drawable.infoicon);
					}
				}
					break;
	...
	private void goPay() {
		...
		try {
			...
			Toast.makeText(MyHotelActivity.this, R.string.remote_call_failed,
	...
	private void readItems(int pageIndex) {
		if (isLogged) {
			...
			api.beginGetMyList(user.getSessionKey(), pageIndex,
					new APICallback<List<RoomOrderInfo>>() {

						@Override
						public void onCompleted(
								APIResultModel<List<RoomOrderInfo>> result) {
							isRunning = false;
							changeLoadingRowText("查看更多...");
							if (result.getSuc()) {
								totalItemCount = result.getTotalItemCount();
								if (totalItemCount == 0) {
									changeLoadingRowText("你还没有预定酒店.");
								} else {
									List<RoomOrderInfo> items = result
											.getItems();
									adapter.addItem(items);
								}
								// list.
							} else {
								Toast.makeText(MyHotelActivity.this, result.getMsg(),
							...
							builder.setPositiveButton("重试",
									new OnClickListener() {
										...
										public void onClick(
												...
												int which) {
											readItems(MyHotelActivity.this.pageIndex);
											dialog.dismiss();
										}
									});
	...
	class PaginationAdapter extends BaseAdapter {
		List<RoomOrderInfo> items;

		public PaginationAdapter() {
			this.items = new ArrayList<RoomOrderInfo>();
		}

		public PaginationAdapter(List<RoomOrderInfo> items) {
			this.items = items;
		}
		...
		public View getView(final int position, View view, ViewGroup parent) {
			RoomOrderInfo item = items.get(position);
			if (view == null)
				view = getLayoutInflater().inflate(R.layout.myhotel_list_item,
						null);
			TextView title = (TextView) view.findViewById(R.id.title);
			title.setText(new StringBuilder(item.getHotelName()).append(" - ")
			...
			inDay.setText(new StringBuilder(item.getCheckInTime().toString(
					"yyyy-MM-dd")).append(" - ")
					.append(item.getCheckOutTime().toString("yyyy-MM-dd"))
					.toString());
					...
					api.beginGetPaySign(user.getSessionKey(), v.getTag()
						...
						public void onCompleted(
							...
							if (result.getSuc()) {
								...
								Toast.makeText(MyHotelActivity.this,
						...
						@Override
						public void onError(String msg) {
							Toast.makeText(MyHotelActivity.this, msg,
									Toast.LENGTH_LONG).show();
						}
		...
		public void addItem(List<RoomOrderInfo> items) {
			this.items.addAll(items);
			this.notifyDataSetChanged();
		}

		public void addItem(RoomOrderInfo item) {
			items.add(item);
			this.notifyDataSetChanged();
		}
	}
	...
	private void cancel() {
		...
		intent.setClass(MyHotelActivity.this, UserCenterActivity.class);
